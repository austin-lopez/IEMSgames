...
State.variables.sheetTitles = ["", "", ""];
State.variables.netIdDatabase = {};
State.variables.currentSection = -1;
State.variables.sheetCount = 3;
State.variables.sheetsLoaded = 0;
...
$.get(sheetURL("DATA", dataSheetID), function (data) {
    ...
    sheets.forEach((element, index) => {
        ...
        $.get(sheetURL("DATA", element), function (data) {
            State.variables.sheetTitles[index] += (data.values.slice(0, 1)[0][0]);
        });
...
window.sendData = function(data) {
    ...
    if (State.variables.netid == "tst0000") return;

    $.ajax({
        url: "https://script.google.com/macros/s/" + scripts[State.variables.currentSection] + "/exec",
        method: "POST",
        dataType: "json",
